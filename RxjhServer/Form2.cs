...
namespace RxjhServer
{
    ...
    public class Form2 : Form
    {
        ...
        private void eval_a(object sender, EventArgs e)
        {
            ...
            foreach (地面物品类 地面物品类 in World.ItmeTeM.Values)
            {
                try
                {
                    ...
                    try
                    {
                        items[0] = BitConverter.ToInt64(地面物品类.物品.物品全局ID, 0).ToString();
                        items[1] = BitConverter.ToInt32(地面物品类.物品.Get_Byte_Item_PID, 0).ToString();
                        items[2] = 地面物品类.物品.Get_Name();
                        items[3] = 地面物品类.Rxjh_Map.ToString();
                        items[4] = 地面物品类.Rxjh_X.ToString();
                        items[5] = 地面物品类.Rxjh_Y.ToString();
                        items[6] = 地面物品类.物品.FLD_MAGIC0.ToString();
                        items[7] = 地面物品类.物品.FLD_MAGIC1.ToString();
                        items[8] = 地面物品类.物品.FLD_MAGIC2.ToString();
                        items[9] = 地面物品类.物品.FLD_MAGIC3.ToString();
                        items[10] = 地面物品类.物品.FLD_MAGIC4.ToString();
                        items[11] = 地面物品类.物品优先权.UserName;
                    }
                    catch
                    {
                        items[0] = BitConverter.ToInt64(地面物品类.物品.物品全局ID, 0).ToString();

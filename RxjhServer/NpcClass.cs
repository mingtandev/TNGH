	...
	public class NpcClass : IDisposable
	{
							...
							if (查找范围玩家(15, value))
							{
								...
								try
								{
									...
									if (RxjhHp <= 0)
									{
										...
										if (value.FLD_VIP == 1)
										{
											num13 += num13 * World.Vip经验倍数百分比;
											num14 += num14 * World.Vip历练倍数百分比;
										}
										if (value.Party_ID != 0 && value.Party_Status == 1)
										{
											if (World.PartyClass.TryGetValue(value.Party_ID, out 组队Class value4))
											{
								...
								if (value.Player_FLD_HP <= 0 || value.PlayerIsDead)
								{
									...
									string str = 坐标Class.getmapname(RxjhMap);
		...
		private void 更新npc复活数据()
		{
			try
			{
				...
				if (FldBoss == 1)
				{
					string text = 坐标Class.getmapname(RxjhMap);
			...
			try
			{
				...
				if (RxjhExp > 0)
				{
					...
					if (yxqname != null)
					{
						...
						if (World.PartyClass.TryGetValue(yxqname.Party_ID, out 组队Class value))
						{
				...
				if (World.Itme.TryGetValue(drop.FLD_PID, out ItmeClass value))
				{
					if (value.FLD_QUESTITEM != 1)
					{
						...
						try
						{
							if (FldBoss == 0)
							{
								地面物品类 = new 地面物品类(array, RxjhX, RxjhY, RxjhZ, RxjhMap, yxqname, 0);
							}
							else
							{
								...
								地面物品类 = ((num != 0) ? new 地面物品类(array, RxjhX - (float)num2, RxjhY - (float)num3, RxjhZ, RxjhMap, yxqname, 0) : new 地面物品类(array, RxjhX + (float)num2, RxjhY + (float)num3, RxjhZ, RxjhMap, yxqname, 0));
							}
							if (地面物品类 == null)
							{
		...
		public long 获得经验()
		{
			...
			if (num > 1 && World.Pk开关 == 0 && !World.isMapPK(_rxjhMap))
			{

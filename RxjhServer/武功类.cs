...
namespace RxjhServer
{
	public class 武功类 : IDisposable
	{
		...
		{
			if (!World.TblKongfu.TryGetValue(wgid, out 武功类 value))
			{
				return false;
			}
			if (value == null)
		...
		{
			foreach (武功类 value in World.TblKongfu.Values)
			{
				if (value.FLD_ZX == 0)
				{
		...
		{
			if (World.TblKongfu.TryGetValue(id, out 武功类 value))
			{
				FLD_AT = value.FLD_AT;
		...
		public static void 学习升天武功书(Players Playe, int FLD_武功类型, int FLD_INDEX)
		{
			...
			{
				Playe.Array_Skill_Book[wg.FLD_武功类型, wg.FLD_INDEX] = new 武功类(wg.FLD_PID);
